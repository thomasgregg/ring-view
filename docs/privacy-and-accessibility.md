...
- Interactive dashboard controls are opt-in. **Door control location** defaults
  to fullscreen only; choosing dashboard and fullscreen deliberately exposes it
  inline. A Live-only action cannot run until the inline Live picture is ready.
  Home Assistant edit and card-picker previews never connect or expose visitor
  actions.
