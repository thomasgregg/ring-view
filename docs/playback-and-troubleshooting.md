...
| Snapshot button is disabled | Confirm that the configured device snapshot camera or Live camera is available and that Home Assistant is connected. |
| Snapshot cannot be saved | Start with `/media/ring-view`. For Home Assistant Container, mount `/media`. Custom folders may need `allowlist_external_dirs`. The editor can validate the path format, but only Home Assistant can test server write access. |
...
| Ring-MQTT recording says it is unavailable | In automatic mode, confirm that Last activity reports a supported category and wait while Ring prepares that category's slot 1. In manual mode, open Event Select and choose the intended event. **Recording Not Found** means Ring-MQTT has no clip for that slot. **Transcoding in Progress** can take 10–15 seconds; keep the viewer open while Ring View waits. Ring View automatically requests the matching **(Transcoded)** option on iPhone/iPad or after a direct Ring URL fails. |
